 ...
 * - Splits the amountWithUnitSelect value into its parts (amount and unit in the format [Amount]-[Unit], E.G. 5-Kilograms) 
 * and assigns them to the request body (req.body)
 * 
...
/**
 * Pre-process hook. This function:
 * - Copies the field (amountWithUnitSelect) value from the request body (req.body) to the form values (req.form.values) 
 ...
 * - Prevents default select component assignment of 'equal' validator to parent component
 * - Resolves required validators and optional configurations for child components
 * - Propagates child component field data and values to the request
 ...
 * - If the parent component has a flagged error, this extends the session model error values with the child components' error values
 * 
 * @param {Object} req - The form's request object
 * @param {Object} fields - The component's child field definitions and configurations
 * @param {string} key - The parent component's key
 ...
 * - Ensures only one error is associated to the components in the request form errors (by setting excess errors' type to null)
 * when either the parent or child components have (jointly) multiple errors in the session model
 * - Given there is no parent component error, inserts one of the child component errors (if any)
 * 
 * @param {Object} req - The form's request object
 ...
 * - Splits the component's value into its parts and assigns them to request object's form values (req.form.values)
 * 
 * @param {Object} req - The form's request object
 * @param {Object} fields - The component's child field definitions and configurations
 * @param {string} key - The parent component's key
 ...
 * - Renders the component's template to a string and assigns the HTML output to the component field in res.locals.fields
 * 
 * @param {Object} req - The form's request object
 * @param {Object} res - The form's response object
 * @param {Object} fields - The component's child field definitions and configurations
