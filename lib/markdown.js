...
module.exports = conf => {
  ...
  return (req, res, next) => {
    ...
    res.locals[config.method] = file => {
      ...
      let nunjucksEnv = req.app.locals?.nunjucksEnv;

      if (!nunjucksEnv) {
        nunjucksEnv = nunjucks;
      }
      const renderedMd = nunjucksEnv.renderString(md, res.locals);
      return new nunjucks.runtime.SafeString(converter.render(renderedMd));
    };
    next();
  };
};
